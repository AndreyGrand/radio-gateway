...
static void write_reg_multi(uint8_t reg,
                            const uint8_t *buf,
                            uint8_t len)
{
    csn_low();
...
void nrf24_init(void)
{
    ...
    write_reg(REG_CONFIG, 0x0C);     // PWR_UP + CRC
    write_reg(REG_EN_AA, 0x00);      // auto-ack OFF
    write_reg(REG_RF_CH, 76);
    write_reg(REG_RF_SETUP, 0x26);   // 250kbps

    write_reg(REG_FEATURE, 0x04);    // dynamic payload
    write_reg(REG_DYNPD, 0x01);

    /* --- addressing --- */
    uint8_t gw_addr[5] = { 'G','W','0','0','1' };
    uint8_t bc_addr[5] = { 'B','C','0','0','0' };

    write_reg_multi(REG_RX_ADDR_P0, gw_addr, 5);
    write_reg_multi(REG_RX_ADDR_P1, bc_addr, 5);
    write_reg_multi(REG_TX_ADDR,    gw_addr, 5);

    write_reg(REG_EN_RXADDR, 0x03);  // P0 + P1
...
uint8_t nrf24_read(uint8_t *buf, uint8_t maxlen)
{
    ...
    write_reg(REG_STATUS, 0x40); // RX_DR clear
    return maxlen;
}
